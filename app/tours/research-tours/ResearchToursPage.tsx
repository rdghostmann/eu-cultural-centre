...
export default function ResearchToursPage() {
  ...
  const researchAreas = [
    {
      title: t("areas.culturalPolicy.title"),
      description: t("areas.culturalPolicy.description"),
      icon: <BookOpen className="h-6 w-6" />,
      duration: t("areas.culturalPolicy.duration"),
      features: t.raw("areas.culturalPolicy.features"),
    },
    {
      title: t("areas.archaeology.title"),
      description: t("areas.archaeology.description"),
      icon: <Microscope className="h-6 w-6" />,
      duration: t("areas.archaeology.duration"),
      features: t.raw("areas.archaeology.features"),
    },
    {
      title: t("areas.ethnography.title"),
      description: t("areas.ethnography.description"),
      icon: <Users className="h-6 w-6" />,
      duration: t("areas.ethnography.duration"),
      features: t.raw("areas.ethnography.features"),
    },
    {
      title: t("areas.archives.title"),
      description: t("areas.archives.description"),
      icon: <Database className="h-6 w-6" />,
      duration: t("areas.archives.duration"),
      features: t.raw("areas.archives.features"),
    },
  ]
    ...
    <div className="flex flex-col overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl mx-auto sm:max-w-4xl" ref={heroRef}>
          <motion.div className="text-center" variants={staggerContainer} initial="hidden" animate={heroInView ? "visible" : "hidden"}>
            <motion.div variants={staggerItem}>
              <Badge className="mb-4 bg-white/20 text-white border-white/30">{t("heroBadge")}</Badge>
            </motion.div>
              ...
              <motion.div whileHover={hoverScale} whileTap={{ scale: 0.95 }}>
                <Button asChild size="lg" className="bg-ecc-green hover:bg-ecc-green/90 w-full sm:w-auto">
                  <Link href="#research-areas">
                    {t("heroPrimary")} <ArrowRight className="ml-2 h-5 w-5" />
                  </Link>
                </Button>
              </motion.div>
              <motion.div whileHover={hoverScale} whileTap={{ scale: 0.95 }}>
        ...
        <div className="max-w-4xl mx-auto">
          <motion.div className="grid grid-cols-2 gap-4 sm:grid-cols-4 sm:gap-6" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            {[
              ...
              { icon: Users, title: t("quickInfo.researchersTitle"), value: t("quickInfo.researchersValue") },
              { icon: Award, title: t("quickInfo.successTitle"), value: t("quickInfo.successValue") },
              { icon: Database, title: t("quickInfo.archivesTitle"), value: t("quickInfo.archivesValue") },
            ].map((item, index) => (
      ...
      <section id="research-areas" className="py-20" ref={areasRef}>
        <div className="container mx-auto">
          ...
          >
            <h2 className="text-3xl lg:text-4xl font-bold text-ecc-slate mb-4">{t("researchAreasTitle")}</h2>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">{t("researchAreasDescription")}</p>
          </motion.div>
            ...
            {researchAreas.map((area, index) => (
              <motion.div key={index} variants={staggerItem} whileHover={hoverLift}>
                <Card className="h-full hover:shadow-lg transition-all duration-300">
                  ...
                  <CardContent>
                    <motion.div className="space-y-3 mb-6" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }}>
      ...
      <section className="py-20 bg-gray-50" ref={supportRef}>
        <div className="container mx-auto">
          <motion.div className="text-center mb-16 px-5" variants={fadeInUp} initial="hidden" animate={supportInView ? "visible" : "hidden"}>
            <h2 className="text-3xl lg:text-4xl font-bold text-ecc-slate mb-4">{t("supportTitle")}</h2>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">{t("supportDescription")}</p>
          </motion.div>

          <motion.div className="grid grid-cols-2 lg:grid-cols-4 gap-6 px-4" variants={staggerContainer} initial="hidden" animate={supportInView ? "visible" : "hidden"}>
            {supportServices.map((service: string, index: number) => (
              <motion.div key={index} className="bg-white flex flex-col items-center justify-center p-6 rounded-lg shadow-sm text-center" variants={staggerItem} whileHover={{ y: -5, boxShadow: "0 8px 25px rgba(0,0,0,0.1)", transition: { duration: 0.2 } }}>
                <motion.div className="w-12 h-12 bg-indigo-100 rounded-full flex items-center justify-center mx-auto mb-4" whileHover={{ scale: 1.1 }}>
      ...
      <section className="py-20">
        <div className="container mx-auto">
          <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">{t("processDescription")}</p>
          </motion.div>

          <motion.div className="grid md:grid-cols-3 gap-8 px-5" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            {[
              { step: "1", title: t("processSteps.step1.title"), description: t("processSteps.step1.description") },
              { step: "2", title: t("processSteps.step2.title"), description: t("processSteps.step2.description") },
              { step: "3", title: t("processSteps.step3.title"), description: t("processSteps.step3.description") },
            ].map((step, index) => (
              <motion.div key={index} variants={staggerItem} whileHover={hoverLift}>
                <Card className="text-center h-full">
                  <CardHeader>
                    <motion.div className="w-16 h-16 bg-indigo-600 rounded-full flex items-center justify-center mx-auto mb-4" whileHover={{ scale: 1.1, rotate: 360 }} transition={{ duration: 0.5 }}>
